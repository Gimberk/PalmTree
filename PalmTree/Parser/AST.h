...
struct ASTNode {
    ...
    virtual Value visit(std::unordered_map<std::string, Value>& variables,
        const std::unordered_map<std::string,
        std::function<Value(const std::vector<Value>&)>>&builtInFunctions) const = 0;
...
struct ExpressionNode : public ASTNode {
    ...
    Value visit(std::unordered_map<std::string, Value>& variables,
        const std::unordered_map<std::string,
        std::function<Value(const std::vector<Value>&)>>&builtInFunctions) const override = 0;
};
...
class LambdaNode : public ExpressionNode, public std::enable_shared_from_this<LambdaNode> {
public:
    ...
    LambdaNode(std::string functionName, std::vector<std::string> arguments,
        std::unique_ptr<ExpressionNode> body)
        : functionName(functionName), arguments(arguments), body(std::move(body)) {}

    Value evaluate(std::unordered_map<std::string, Value>& variables,
        const std::unordered_map<std::string,
        std::function<Value(const std::vector<Value>&)>>&builtInFunctions) const override {
        return Value(shared_from_this()); // this needs to be created as a sharedptr in parser.cpp
    }
    ...
    Value visit(std::unordered_map<std::string, Value>& variables,
        ...
        for (std::string arg : arguments) variables.erase(arg); // very safe way to clean out local scope
        return exprResult;
    }
...
struct FunctionCallNode : public ExpressionNode {
    ...
    Value evaluate(std::unordered_map<std::string, Value>& variables,
        ...
        for (auto& var : variables) {
            ...
            // this means that a global var already exists with the same name as the arg
            for (std::string arg : lambda->arguments) if (variables.find(arg) != variables.end())
                throw std::runtime_error("Variable with identifier already exists!");
